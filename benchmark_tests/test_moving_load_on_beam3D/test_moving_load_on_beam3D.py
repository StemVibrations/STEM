...
def test_stem():
    ...
    beam_material = EulerBeam(ndim, YOUNG_MODULUS, POISSON_RATIO, DENSITY, CROSS_AREA, I33, I22,1)
    name = "beam"
    structural_material = StructuralMaterial(name, beam_material)
    # Specify the coordinates for the beam: x:1m x y:0m
    beam_coordinates = [(0, 0, 0),(0.5,0,0), (1, 0, 0)]
    # Create the beam
    ...
    # Define moving load
    moving_load = MovingLoad(load=[0.0, -10000, 0.0], direction=[1, 1, 1],
                             velocity=1.0, origin=[0.0, 0.0, 0.0], offset=0.0)

    # point_load = PointLoad(value=[0,-10000,0], active=[True,True,True])

    # model.add_load_by_geometry_ids([1], moving_load, "moving_load")
    model.add_load_by_geometry_ids([1,2], moving_load, "moving_load")

    # Define rotation boundary condition
    # rotation_boundaries_parameters = RotationConstraint(active=[True, True, True], is_fixed=[False, False, False],
    #                                                     value=[0, 0, 0])
    ...
    # model.add_boundary_condition_by_geometry_ids(0, [1,2], rotation_boundaries_parameters, "rotation")
    model.add_boundary_condition_by_geometry_ids(0, [1,3], displacementXYZ_parameters, "displacementXYZ")
    ...
    time_integration = TimeIntegration(start_time=0.0, end_time=1., delta_time=0.01, reduction_factor=1.0,
    ...
    solver_settings = SolverSettings(analysis_type=analysis_type, solution_type=solution_type,
                                    ...
                                    convergence_criteria=convergence_criterion, rayleigh_k=0.01,
                                    rayleigh_m=0.01)
