...
@pytest.mark.parametrize("mesh_order, case_name", [
    (1, "hex8n"),  # 8 noded elements
    (2, "hex20n")])  # 20 noded elements
def test_stem(mesh_order, case_name):
    ...
    # Set up problem data
    problem = Problem(problem_name="test_lysmer_boundary_column3d_hex",
                      number_of_threads=1,
    ...
    input_folder = f"benchmark_tests/test_lysmer_boundary_column3d_hexa/{case_name}"
    ...
    result = assert_files_equal(
        f"benchmark_tests/test_lysmer_boundary_column3d_hex/{case_name}_output/output_vtk_porous_computational_model_part",
