...
def test_interface_2d(test_name, roller, stiffness, interface_test_results):
    ...
    load_coordinates = [(1.0, 1.0, 0), (1.0, 1.25, 0)]

    # Add line load
    line_load = PointLoad(active=[True, True, True], value=[100, 0, 0])
    ...
    model.add_output_settings_by_coordinates(
        coordinates=[(1.0, 1.2, 0), (1.0, 0.75, 0)],
    ...
    if os.path.exists(output_file):
        ...
        interface_test_results[test_name] = output_data
    else:
        interface_test_results[test_name] = None
    assert interface_test_results is not None, "No results found"
...
def test_interface_2d_comparison(interface_test_results):
    ...
    assert np.isclose(bottom_node_disp_high_stiffness_roller_horizontal, 0.0)
    assert not np.isclose(bottom_node_disp_low_stiffness_no_roller_horizontal, 0.0)
    assert not np.isclose(bottom_node_disp_high_stiffness_no_roller_horizontal, 0.0)
    # Compare the interface on and off cases
    assert top_node_disp_low_stiffness_roller_horizontal > top_node_disp_high_stiffness_roller_horizontal
    assert top_node_disp_low_stiffness_no_roller_horizontal > top_node_disp_high_stiffness_no_roller_horizontal
    assert top_node_disp_low_stiffness_roller_vertical > top_node_disp_high_stiffness_roller_vertical
    assert top_node_disp_low_stiffness_no_roller_vertical > top_node_disp_high_stiffness_no_roller_vertical
