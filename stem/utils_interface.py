...
class UtilsInterface:
    ...
    """
    @staticmethod
    def get_quadratic_order_nodes(nodes_stable_parts: List[int],
                                  nodes_for_element: List[Node]) -> List[int]:
        """
        ...
        equal_node = {node.id: node for node in nodes_for_element if node.id not in list(initial_nodes.keys()) and np.allclose(node.coordinates, list(initial_nodes.values())[-1].coordinates)}
