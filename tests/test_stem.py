...
class TestStem:
    ...
    @pytest.fixture(scope="function", autouse=True)
    def create_model_with_line_output(self) -> Model:
        """
        Create a default model for the tests
    ...
    def test_finalisation_of_stages(self, create_model_with_line_output: Model):
        """
        Test the __set_output_name_new_stage method of the Stem class with JsonOutputParameters. It checks if the output
        name is set correctly for the new stage.
        ...
        # initialize the stem object
        stem = Stem(initial_stage=create_model_with_line_output, input_files_dir="input_files")

        stem.write_all_input_files()
        # run a stage
        stem.run_calculation()

        # in the next code blocks, local modification are made to the model to
        # ensure errors are raised appropriately from the Model.finalise() method
        # the stage is deep-copied everytime to avoid removing the original values

        # set mesh of the output process model part (line output) to None
        # stage = deepcopy(stem.stages[0])
        # stage.process_model_parts[-1].mesh = None
        # msg = "process model part has not been meshed yet!"
        # with pytest.raises(ValueError, match=msg):
        #     stage.finalise(working_folder="input_files")

        stage = deepcopy(stem.stages[0])
        stage.output_settings[-1].output_name = None
        ...
        stage = deepcopy(stem.stages[0])
        stage.output_settings[-1].output_name = "json_nodal_displacements"

        msg = (
            "No JSON file is found in the output directory for path: input_files\\dir_test\\json_nodal_displacements. "
        ...
        stage = deepcopy(stem.stages[0])
        stage.output_settings[-1].part_name = None
        msg = "The output model part has no part name specified."
        with pytest.raises(ValueError, match=msg):
            stage.finalise(working_folder="input_files")

        stage = deepcopy(stem.stages[0])
        stage.output_settings[-1].part_name = "part 404"
        msg = "No model part matches the part name specified in the output settings."
        with pytest.raises(ValueError, match=msg):
            stage.finalise(working_folder="input_files")
        # modify the output settings to check errors are raised
        # set the output parameters of the new stage
        # new_stage.output_settings[0].output_parameters = MagicMock(spec=JsonOutputParameters)
        # new_stage.output_settings[0].output_name = "json_output"
        # stem._Stem__set_output_name_new_stage(new_stage, 3)
