...
class GmshIO:
    ...
    def create_point(self, coordinates: Iterable[float], element_size: float) -> None:
        """
        Creates points in gmsh.

        Args:
            coordinates (Iterable[float]): An Iterable of point x,y,z coordinates.
    ...
    def create_line(self, point_ids: Iterable[int]) -> None:
        """
        Creates lines in gmsh.

        Args:
            point_ids (Iterable[int]): A list of point tags in order.
    ...
    def create_surface(self, line_ids: Iterable[int], name_label: str) -> int:
        """
        Creates curve and then surface in gmsh by using line tags.

        Args:
            line_ids (Iterable[int]): A list of line tags in order.
    ...
    def create_volume_by_extruding_surface(self, surface_id: int, extrusion_length: List[float,float,float]) -> None:
        """
        Creates volume by extruding a 2D surface

        Args:
            surface_id (int): The surface tag.
            extrusion_length (List[float,float,float]): The extrusion length in x, y and z direction.
    ...
    def make_geometry_2D(self, point_coordinates: Iterable[float], point_pairs: List,
        ...
        Args:
            point_coordinates (Iterable[float]): A list of point coordinates.
            point_pairs (List[List[int,int]]): A list of point tags of two consecutive points in an array.
    ...
    def make_geometry_3D(self, point_coordinates: Iterable[float], point_pairs: List, mesh_size: float,
                         extrusion_length: List[float, float, float], name_label: str) -> None:
        """
        Creates 3D geometries by extruding the 2D surface

        Args:
            point_coordinates (Iterable[float]): Geometry points coordinates.
            point_pairs (List[List[int,int]]): A list of point tags of two consecutive points in an array.
    ...
    def generate_point_pairs(self, point_coordinates: Iterable[float]) -> List[List[int, int]]:
        """
        Generates pairs of point IDs which form a line

        Args:
            point_coordinates (Iterable[float]): A list of geometry points coordinates.
    ...
    def generate_gmsh_mesh(self, point_coordinates: Iterable[float], extrusion_length: List[float, float, float],
                           mesh_size: float, dims :int, name_label: str, mesh_name: str,
                           mesh_output_dir: str, save_file: bool = False, open_gmsh_gui:bool = False):
        """
        ...
        Args:
            point_coordinates Iterable[float]: User input points of the surface as a list of tuples.
