...
def test_stem():
    ...
    # Define load
    load = PointLoad(active=[True, True, True], value=[force, 0, 0])
    load_coordinates = [top_coordinate]
    ...
    model.add_boundary_condition_by_geometry_ids(1, [3], no_displacement_parameters, "base_fixed")
    model.add_boundary_condition_by_geometry_ids(2, [1], no_vertical_displacement, "no_vert_displacement")
    ...
    # --------------------------------
    input_folder = "benchmark_tests/test_vibrating_dam/inputs_kratos"
    ...
    calculated_output_file_name = Path(input_folder) / "json_output_top.json"
    with open(calculated_output_file_name, 'r') as f:
        calculated_output = json.load(f)

    expected_output_file_name = Path("benchmark_tests/test_vibrating_dam/_output") / "expected_json_output_top.json"
    ...
    if SHOW_RESULTS:
        ...
        calculated_horizontal_displacement = calculated_output["NODE_2"]["DISPLACEMENT_X"]
        ...
        for i, expected_natural_frequency in enumerate(expected_natural_frequencies):
            plt.axvline(expected_natural_frequency,
                        color='g',
                        linestyle='--',
                        label=f'Expected {i+1}st natural frequency')

        plt.xlabel('Frequency [Hz]')
        plt.ylabel('Power Spectral Density')
        plt.title('Power Spectral Density of Horizontal Displacement at Load Point')
        plt.xlim([0, 20])
        plt.grid()
