...
from nightly_build.stem_runners.compare_analytical import compare_pekeris, compare_strip_load_2D, \
      compare_strip_load_3D, compare_wave_propagation, compare_sdof
...
f_log = open("nightly_build/stem_runners/runner_log.txt", "w")
...
# f_log.write(f"Pekeris runner time: {time() - t_ini:.2f} seconds\n")

# t_ini = time()
# run_strip_2D(Path("./strip_2D_tmp"))
# f_log.write(f"Strip 2D runner time: {time() - t_ini:.2f} seconds\n")
# t_ini = time()
# run_strip_3D(Path("./strip_3D_tmp"))
# f_log.write(f"Strip 3D runner time: {time() - t_ini:.2f} seconds\n")
# t_ini = time()
# f_log.write(f"One Dim Wave Prop runner time: {time() - t_ini:.2f} seconds\n")
# t_ini = time()
# run_moving_load(Path("./moving_load_halfspace_tmp"))
# print(f"Moving Load Halfspace runner time: {time() - t_ini:.2f} seconds")

# compare_analytical
compare_sdof(r"sdof_tmp/sdof_results.json", r"nightly_build/sdof/sdof_time_history.pdf")
