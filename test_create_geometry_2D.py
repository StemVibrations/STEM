...
import sys
import pytest
from create_geometry_3D import make_geometry_2D, create_point_pairs
...
def test_make_geometry():
    ...
    gmsh.initialize()
    make_geometry_2D(input_points, point_pairs, mesh_size, name_label)
    gmsh.model.geo.synchronize()
    gmsh.model.mesh.generate(dims)
    coord, node_tags, elem_types, elem_tags, node_tag_1D, node_tag_2D = extract_mesh_data(dims)
