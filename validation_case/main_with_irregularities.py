...
def run_model():
    ...
    ballast_geometry = [(0, 0, 0), (4.4, 0, 0), (2.4, ballast_height, 0), (0, ballast_height, 0)]

    soil_properties = {
        "soil_layer_1": [soil_layer_1, 2010, 3.16e8, 0.313],
        "soil_layer_1b": [soil_layer_1b, 2010, 3.16e8, 0.313],
    ...
    def get_wheel_configuration(
        start_position_cart: float,
        cart_to_cart_distance: float,
    ):

        interdistance_wheels = []
        for boogie_distance in bogie_distances:
            for wheel_distance in wheel_distances:
    ...
    wheel_configuration = get_wheel_configuration(start_position_cart=0, cart_to_cart_distance=5.2)

    # define uvec parameters
    uvec_parameters = {
        "n_carts": n_carts,  # number of carts [-]
        ...
        "bogie_distances": bogie_distances,  # distances of the bogies from the centre of the cart [m]
        "bogie_inertia": (0.31e3) / 2,  # inertia of the bogie [kgm2]
        "bogie_mass": (6e3) / 2,  # mass of the bogie [kg]
        "wheel_distances": wheel_distances,  # distances of the wheels from the centre of the bogie [m]
        ...
        # gradually increased [-]
        "velocity": 27.7,
        "current_position": wheel_configuration,
        "wheel_configuration": wheel_configuration,
        "irr_parameters": {  # irregularities in the tracks using a
            "Av": 0.2095e-04,
            "seed": 14
        }
    }
    ...
    model_stage_1.set_element_size_of_group(1, "soil_layer_1b")
    model_stage_1.set_element_size_of_group(1, "soil_layer_2")
    # model_stage_1.set_element_size_of_group(0.5, "ballast")
    ...
    model_stage_1.add_output_settings_by_coordinates(
        ...
        coordinates=[
            (output_distance, 0, 0),
            # (output_distance, 0, round(model_length/4, 3)),
            (output_distance, 0, model_length / 2),
            # (output_distance, 0, round(3*model_length/4, 3)),
            (output_distance, 0, model_length)
        ],
        # coordinates=[
        #     (0, 0, half_model),
        #     (output_distance, 0, half_model),
        #     (model_width, 0, half_model)
        # ],
    ...
    input_folder = "inputs_kratos_irregularities_vel_100kmh"
    # model_stage_1.show_geometry()
    ...
    # model_stage_2.project_parameters.settings.rayleigh_k = 2.83e-4
    # model_stage_2.project_parameters.settings.rayleigh_m = 8.94
    ...
    # model_stage_2.generate_mesh(save_file=True, open_gmsh_gui=True)
