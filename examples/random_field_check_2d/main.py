...
mesh_name = "simple_2d_mesh_rf.mdpa"

working_folder = os.getcwd()
main_folder = os.path.join(working_folder,"examples/random_field_check_2d")
...
model.add_random_field(part_name="layer1", property_name="YOUNG_MODULUS", cov=cov,
                       v_scale_fluctuation=v_scale_fluctuation, anisotropy=anisotropy,
                       angle=angle, seed=seed, model_name=model_name)

x_in_load = (1-perc_load)/2*b
x_end_load = (1+perc_load)/2*b

load_coordinates = [(x_in_load, h, 0.0), (x_end_load, h, 0.0)]
line_load = LineLoad(active=[False, True, False], value=[0, -1, 0])
